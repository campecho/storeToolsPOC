...
/**
 * Line decorations, merged with the math from the publisher prototype (its
 * lineDecor.test.ts, adapted to this app's px-at-zoom-1 stroke widths —
 * the prototype's 0.75pt floor is exactly 1px here): dash patterns scale
 * with stroke width, arrowhead length is a per-size base plus one stroke
 * width, and arrowhead geometry is built from the head's pointing direction
 * at the tip. All constants are Publisher-parity guesses carried from the
 * prototype — the tests pin them down exactly.
 */
...
describe("dashPatternIn", () => {
  ...
  it("floors the stroke width at 1px so hairlines stay visible", () => {
    expect(dashPatternIn("dashed", 0.1)).toEqual(dashPatternIn("dashed", 1));
  });
});
...
describe("headLengthIn", () => {
  ...
  it("adds one stroke width in inches to the base", () => {
    expect(headLengthIn("m", 9.6)).toBeCloseTo(0.14 + 0.1, 10);
  });
});
