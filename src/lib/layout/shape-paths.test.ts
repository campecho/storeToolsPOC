...
import type { PathSeg } from "@/schema";
import {
...
/**
 * Shape-tool path builder invariants, merged with the builders from the
 * publisher prototype (its shapePaths.test.ts, adapted to this schema's
 * `type` field): every builder emits one closed, clockwise subpath (single
 * M … Z) with all coordinates — control points included — normalized inside
 * [0, 1], and each shape's landmark vertices land exactly where the
 * prototype's tool contracts promise.
 */
...
describe("outlineOvershoot", () => {
  it("reports a callout's tail tip — the one point any builder puts outside its box", () => {
    expect(outlineOvershoot({ type: "callout", tailTip: { x: 1.4, y: -0.3 } })).toEqual([
      { x: 1.4, y: -0.3 },
    ]);
    expect(outlineOvershoot({ type: "callout" })).toEqual([tailTipFor("bottom-left")]);
  });
  ...
  it("reports nothing for every kind whose outline stays inside its box", () => {
    for (const shape of [
      ...
      expect(outlineOvershoot({ type: shape })).toEqual([]);
    }
  });
});
...
describe("shapeShading", () => {
  it("gives the banner its folds", () => {
    expect(shapeShading({ type: "banner" }, 4, 1)).toEqual(
  ...
  it("gives every other kind nothing — one tone is the whole shape", () => {
    for (const shape of [
      ...
      expect(shapeShading({ type: shape }, 4, 1)).toEqual([]);
    }
  });
});
...
describe("starInnerArmPoint", () => {
  it("sits on starPath's first inner vertex — the point the adjust handle rides", () => {
    const ring = starPath(5, 0.4);
    const inner = ring[1];
    const p = starInnerArmPoint(5, 0.4);
    expect(inner?.c).toBe("L");
    if (inner?.c === "L") {
