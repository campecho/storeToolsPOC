...
/**
 * Color arithmetic (redesign plan Phase 12): hex parsing and formatting,
 * channel scaling between the model's 0–1 and what people type (percent for
 * CMYK, 0–255 for RGB), the HSV helpers the picker's visual field needs, and
 * the NAIVE device conversions. The naive formulas are the fallback and the
 * test oracle only — the canvas previews through the GRACoL-derived tables in
 * proof.ts, because (1 − c)(1 − k) renders 100% cyan as pure #00ffff, which
 * no press prints.
 *
 * Pure functions, no DOM, no React — shared by the layout editor, the photo
 * editor, and the unit tests.
 */
...
/** HSV for the visual picker: h in [0, 360), s and v in [0, 1]. */
...
/* ── Constructors (what defaults and tests read naturally) ── */
...
export function cmykPercent(c: number, m: number, y: number, k: number): ColorValue {
  return { space: "cmyk", values: [clamp01(c / 100), clamp01(m / 100), clamp01(y / 100), clamp01(k / 100)] };
}

/* ── Hex ── */

/** Six hex digits, with or without `#`, any case. Three-digit shorthand and
    eight-digit alpha are rejected: neither the model nor print carries alpha,
    and silent coercion hides typos. */
...
/** `#rrggbb`, lowercase — the codebase compares hex case-insensitively and
    the import fidelity harness lowercases, so lowercase is the canonical form. */
...
/** A hex string as an rgb literal. Malformed input resolves to black — the
    soft-failure rule the migration and the importer need (a v3 document's
    color was an unvalidated string). */
...
/** The inverse with a K-only black: any color whose brightest channel is 0
    is 0/0/0/100, never rich black (designStudio's rule, kept). */
...
/* ── HSV (the picker's visual field lives in screen HSV) ── */
...
/** Same space and channels within 8-bit / integer-percent precision — how
    the picker decides which preset swatch is active. */
