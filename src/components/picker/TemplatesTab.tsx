...
/**
 * Templates tab of the picker at `/` (redesign plan Phase 11 — figma
 * "Publisher - select a template"): Filters panel (search + categories + sort)
 * beside the Template Explorer's carousel rows; selecting a card opens the
 * configuration drawer (dimensions, orientation, margins, bleed) whose Create
 * Document starts a fresh publication from the choice and lands in the editor.
 * The picker's tab strip renders atop the Filters panel via `tabsSlot`
 * (matching the figma, where the tabs sit inside the left column).
 */
...
export function TemplatesTab({ tabsSlot }: { tabsSlot?: React.ReactNode }) {
