...
export function QuickJumpRow() {
  return (
    <div className="flex gap-[14px] border-b border-[#ececec] px-[26px] py-4">
      {TOOLS.map(({ icon: Icon, label, desc }) => (
        <div
          key={label}
          className="flex flex-1 cursor-pointer items-center gap-3 rounded-[10px] border border-[#e6e6e6] bg-white px-[15px] py-[13px] hover:border-brand hover:shadow-[0_2px_10px_rgba(0,0,0,.06)]"
        >
