...
/**
 * The tool registry (PLAN.md §4). Population order follows the build: a tool
 * enters the registry when its contract is authored, and its tier reports its
 * actual build status (§3). Seeded here with the two navigation tools the
 * canvas foundation makes LIVE; the full ~24-tool layout set and ~8-tool
 * photo set land with their Phase B groups.
 */
...
export const toolRegistry: readonly ToolContract[] = [zoomTool, panTool];
