...
**Revision (v1.6, 2026-07-06):** **P2 shipped** — the schema-v2 content core. (1) **Schema v2 is live** (`version: 2`): per-run text (`paragraphs[].runs[]` with font + ink color), paragraph indents, optional text-frame `inset` and `vAlign`, and the `path` object type (normalized M/L/C/Z segments) — with a real **v1→v2 migration on load** (`src/lib/schema/layout-v1.ts`), not drop-and-reseed: persisted documents keep opening. (2) **The §10.5 font library is real**: eight libre stand-in families vendored as self-hosted WOFF2 (no CDN) with lazy FontFace registration and measurement re-run on load; the remap table is data (`src/lib/import/font-remap.ts`) grown from the actual corpus — Calibri stays *Calibri* rendered via Carlito, HelveticaNeue LT cuts → Libre Franklin (tier 2), Goudy Old Style via Sorts Mill Goudy, and Wingdings checkbox glyphs translate to Unicode (✔/✘/☑) with a report note. (3) **The editor renders and edits runs**: the canvas draws real paragraph/run DOM (mixed sizes, faces, colors — the corpus's white-on-dark labels finally render white), and the contentEditable overlay seeds styled runs and parses the browser-mutated DOM back, so editing an imported frame preserves its styling. (4) **Corpus verified live**: production_checkpoint_labels went from 34 converted / 158 degraded (P1) to **176 / 16** with all 128 vector shapes as real paths. Editing gaps stay honest: styling applies frame-wide (selection-scoped styling is a later slice), and there's no color-picker or path-node editing UI yet.

**Revision (v1.7, 2026-07-06):** **P3 shipped** — image extraction. The corpus corrected the spec before a line was written: real Publisher images arrive as **bitmap fills** (`draw:fill: bitmap` + `draw:fill-image` base64 on `setStyle`, stretched onto axis-aligned rectangle-polygons), not as `drawGraphicObject` embeds — both paths are now handled. Extracted images dedupe by content hash into the L8 asset store (the labels file applies ONE logo to 16 frames → one asset), with sniffed-not-declared MIME, parsed PNG/JPEG/GIF dimensions, and a new picture-frame `fit` mode (`stretch` on import — Publisher scales to the frame; L8 uploads keep `cover`). Transport: the import response carries `assets` as base64 payloads (report.ts contract) that the client decodes to Blobs and seeds into IndexedDB before the document opens — a production service swaps to asset URLs behind the same seam. Honest degradations remain: WMF/EMF/TIFF can't render in a browser `<img>` → placeholder + format-named note, no asset; bitmap fills on non-rectangular geometry keep the vector shape unfilled + note (image-clip is backlog). Live corpus: checkpoint labels **192/192 converted, 0 degraded** (was 34/158 at P1, 176/16 at P2); business card 13/13 with its 600×434 JPEG extracted.

**Revision (v1.8, 2026-07-06):** **P4 shipped** — the associate-facing review layer. (1) **Import report panel**: a side-panel "Review" tab (`ImportReportPane`) that renders the fidelity report — remapped fonts, overset frames, and notes grouped by tier — with every object-anchored row deep-linking to select the frame and jump to its page; it's auto-opened on import when there's something to review, and the stopgap banner now has a "View report" button. (2) **Overset check**: a headless measurement (`overset.ts` + `OversetCheck`) that, after import and gated on `document.fonts.ready` (§10.5), measures every imported text frame across all pages against the canvas's exact layout and records overflowing ids — the #1 post-conversion problem now surfaced, not silent. (3) **`.puz` handling**: a pure-TypeScript MSCF/CAB reader (`cab.ts`) unpacks Publisher pack-and-go — STORED + MSZIP (cross-block dictionary continuation) supported, Quantum/LZX honestly rejected — re-sniffing the extracted bytes and re-entering the pipeline; no new native dependency, verified live on a real 100 KB `.pub` in a multi-block CAB. Built by three parallel Opus agents against store seams frozen up front; supervised, reviewed, and gated (369 unit / 80 e2e / live corpus + live `.puz`). Honest limit recorded: no real `.puz` sample exists to test against, so MSZIP byte-compatibility with Publisher's own packer is unconfirmed — unsupported compressions fail with clear guidance rather than silently.

**Revision (v1.9, 2026-07-06):** **P5 shipped — the P-tranche exit gate is met.** The ≥90% element-level fidelity target is now **measured, not asserted**. (1) **Fidelity harness** (`src/lib/import/fidelity.ts` + `corpus-fidelity.test.ts`): scores the pipeline's `LayoutDocument` against `pub2xhtml`'s reference render of the same file — an independent consumer of the same libmspub parse — across the seven Markzware categories (page size, position, color, font+remap, text attributes, text flow, images). Deterministic and binary-free (reads checked-in traces + new checked-in reference renders under `fixtures/pub-refs/`); it gates every category at ≥90% combined and prints a per-file × per-category scorecard. **Measured: 100% in all seven categories** on the content-bearing corpus (position 208/208, color 398/398, font/textAttrs 166/166, flow 75/75, images 24/24, pageSize 7/7), extras 0, unmatched 0 — a *falsifiable* 100%, with 38 unit tests exercising each category going red. `business_card_template_10up` is scored separately: a 0-byte reference, an empty page, and a tier-3 flag all agreeing rather than inflating the denominator. (2) **The missing §10.1 control, built**: CPU/memory rlimits on the `pub2raw` subprocess via a `prlimit` wrapper (soft:hard CPU gap so a genuine overrun raises a classifiable SIGXCPU → `resource-limit` outcome; RLIMIT_AS bound enforced, honestly labelled parse-failed; unwrapped-and-reported where `prlimit` is absent). (3) **Adversarial suite** proving the POC-enforced controls — timeout-kill, scratch-jail wipe, size cap, extension-lie/OLE2-decoy rejection, CAB decompression-bomb + entry-storm + path-traversal-name handling, sniff edges, and a live-gated fuzz smoke. (4) **CI wired** (`.github/workflows/ci.yml`): a binary-free `checks` lane (lint/typecheck/unit+fidelity), a Playwright `e2e` lane, and a `live-import` lane that installs libmspub-tools, regenerates the corpus (`scripts/refresh-corpus.mjs`), gates on drift, and runs the suite with `STP_LIVE_IMPORT=1`. Built by three parallel Opus agents against frozen script/metric/env seams; supervised, reviewed, and gated (450 unit / 80 e2e / live lane green). Honest framing: 100% means our pipeline drops and corrupts nothing relative to what libmspub extracts — the "nothing silent" half of the gate — not an independent oracle for Publisher's own renderer; and the textFlow score counts the documented, report-noted Wingdings→Unicode substitution as a match (without it, 78.7%).

**Revision (v1.10, 2026-07-06):** **Import autofit shipped** — §10.5's earmarked "shrink to fit", promoted from quick-action to automatic after the corpus business card showed the failure mode live: geometry, insets, and sizes import faithfully, but the tier-2 stand-in (Sorts Mill Goudy for Goudy Old Style) renders 1–3% wider, wrapping lines Publisher kept single (Publisher's own shrink-on-overflow — unexposed by libmspub — was masking it). The fix mirrors Publisher, measurement-driven: after the font-gated overset pass, `computeAutofit` finds the largest 1%-quantized render scale (floor **0.88**) that fits each flagged frame, measured always from declared run sizes (idempotent; late-font re-measures converge). Within the floor band the scale auto-applies **silently-but-reported** — schema v2 gains optional `text.fontScale` (render-time only; declared sizes stay the source of truth, so it round-trips and reverses), and every applied scale is a deep-linkable "Auto-fitted" note in the Review panel. Beyond the floor, the frame renders at its true declared size and stays badged. Verified live on the card: 5 frames auto-fit (97/96/90/89/89%), address/tagline/email one-line complete again; the one genuinely-overfull frame (truncated in Publisher too) stays badged. Also fixed en route: the canvas overflow badge's fixed 1px cushion flipped verdicts at high zoom on borderline frames the import mirror accepted — the cushion now scales with zoom while the mirror's semantics are untouched. Editing/resizing does not re-run autofit (the live badge is the honest fallback); a re-fit action is a later slice.

**Revision (v1.12, 2026-07-07):** cross-tool scope recorded from the **photo-editor design handoff** (`docs/handoff/photo-editor/`, Section F): editing a picture placed on a page ships as **F1 + F2 together** — F1 = a "Picture" inspector tab on picture selection (placed-size DPI check, Auto-enhance, Brightness/Contrast, Crop to frame, "Edit in Photo Editor →"); F2 = the round-trip into the Photo Editor that returns the edit as **one named, revertable history step** on the placed instance (picture frames gain an optional, additive `photoEdit { recipe, originalAssetId }`). F1 and the layout-side revert land from the §6 backlog; the photo-editor side (return banner, Done/Cancel, Export suppression) is **PE8** in `docs/PHOTO_EDITOR_IMPLEMENTATION_PLAN.md`. Also noted there: the photo handoff specifies a **Pro** level for that tool — a deliberate contrast with this editor's two-level model (v1.3), not a reversal of it.

**Revision (v1.11, 2026-07-07):** **Field-corpus fixes** — the first user-contributed corpus file (`ecl_workbook.pub`, a 39-page, 508-object, screenshot-heavy training workbook) reproduced three reported issues and surfaced a fourth; all four are addressed and the file now scores 100% in the harness. (1) **Flip correction (`escher.ts` + `flip-correct.ts`)**: the conversion toolchain folds Publisher's Escher flip flags into `librevenge:rotate` for text and discards the mirror — a flipped box, which Publisher renders upright, imported upside down (56 frames: the title on every page, footers on odd pages). The importer now reads the flip flags, true rotation, and anchors straight from the `.pub` (pure-TS CFBF + Escher walk; Publisher's client anchor is fieldId/EMU pairs with a page-center origin), correlates by geometry, and restores upright text with deep-linkable `kind:"corrected"` notes — live mode only, unreadable Escher proceeds uncorrected, uncorrelated 180s stay (tent cards are real). (2) **Arc paths convert as cubics (`arc.ts`)**: two-arc callout ellipses previously degraded to endpoint-hull bboxes — height 0, invisible; SVG F.6.5 conversion with quadrant-boundary splitting renders them real and took the file's position score from 91.9% to 100% (41 degradations → 0). (3) **Wrap-overlap notes**: libmspub emits no text-wrap data, so body copy Publisher wrapped around pictures imports covered by them; a z-order-aware pass (picture above text, ≥20% coverage) now announces each such frame (21 on this file, zero false positives across the rest of the corpus). True wrap remains K-tranche text-layout work. (4) **Page-number fields** arrive as literal `#` (no insertField in the trace) — now **substituted at import**: standalone `#` tokens in header/footer-band frames are replaced with each page's real number (shared rule in `page-number.ts`, mirrored on the fidelity harness's reference side like the Wingdings precedent; glued `#`s such as "#1" are content and stay), reported as one aggregate `kind:"corrected"` note. Corpus and harness ingest the file (fidelity denominators more than tripled: position 716/716); report panel gains a "Corrected" group.
...
| Placed-picture quick fixes + Photo Editor round-trip *(v1.12, photo handoff §F)* | Inspector "Picture" tab + revert here; the photo-editor side is PE8 in `docs/PHOTO_EDITOR_IMPLEMENTATION_PLAN.md` | F1: placed-size DPI check, Auto-enhance, Brightness/Contrast, Crop to frame, "Edit in Photo Editor →" (consumes the photo tool's `adjust-math`/`geometry`, not a fork); F2 revert: optional additive `photoEdit {recipe, originalAssetId}` on picture frames, "Revert photo edits" as one named step |
...
|---|---|---|
| **Asset store** *(✅ upload L8; `.pub` extraction shipped P3, v1.7)* | Picture import (**upload path shipped early in L8**, v1.3; `.pub` extraction P3), Konva image render | `assets: Record<id, { mime, width, height, source }>` — `source` is an IndexedDB blob ref client-side or a server asset URL for imported docs; document JSON stays small. Import dedupes by content hash (one id per distinct payload) and transports bytes as base64 in the response (`assets` half of the report contract) — production swaps to URLs behind the same seam |
| **Picture frame binding** *(✅ `assetId` L8; `fit` shipped P3, v1.7)* | Same | `Frame(type:'picture')` gains `assetId: string \| null` (null = the POC's gray placeholder) + `fit: 'cover' \| 'stretch' \| 'contain'` (absent = cover; imports emit stretch — Publisher scales to the frame) |
| **Path object** *(✅ shipped, P2)* | `.pub` polygons/freeform shapes (P2) | `{ type:'path', x, y, w, h, rotation, d: PathSeg[], fill, stroke }` — normalized segment array (0–1 in the frame box, so move/resize tooling works unchanged), not raw SVG strings |
| **Per-run text** *(✅ shipped, P2 — v2 is live, `version: 2` + v1→v2 migration)* | `.pub` character formatting (P2), Text band v2 | `text.paragraphs: { align, lineSpacing, indent?, firstLineIndent?, runs: { text, font, color }[] }[]` — supersedes the per-frame flat model; frame-level "current style" is DERIVED (`textSummary`), never stored |
| **Text color** *(✅ shipped, P2)* | Per-run model, import fidelity | `color` joins the run props (the v1 schema styled the frame, not the ink) — plus optional frame `inset` and `vAlign` (both import-fed) |
...
- **Out-of-process subprocess with caps**: per-file size cap, wall-clock timeout with kill (a hang *is* a finding), CPU/memory rlimits via a `prlimit`/`ulimit` wrapper. *(Built in P5: `prlimit --cpu=<soft:hard> --as=<bytes> -- pub2raw`; the CPU soft:hard gap forces a classifiable SIGXCPU → `resource-limit`. Absent-`prlimit` hosts run unwrapped and say so via the GET diagnostic — never a silent claim.)*
...
| `drawPolygon` / `drawPolyline` / `drawPath` | `path` object (§9) — normalized M/L/C/Z segments (Q lowered to C exactly); *shipped in P2*; unmodeled verbs (arcs) still fall back to bbox + note | 1 (P2, live) |
...
*Built (v1.8, P4):* the report is a real side-panel **"Review" tab** (`ImportReportPane`), shown only after an import and auto-opened when there's anything to review — remapped fonts (source → mappedTo + reason), overset frames, and notes grouped by tier (3 = not converted, 2 = simplified); every object-anchored row **deep-links** (`setActivePage` + `setSelection`) and the stopgap banner gained a "View report" button. The **overset check** (`src/lib/import/overset.ts` + the headless `OversetCheck`) measures every imported text frame across all pages in a detached container that mirrors `TextFrameNode`'s exact layout (insets, wrap, subpixel cushion), **gated on `document.fonts.ready`** and re-run when a late webfont changes the verdict (§10.5), writing `report.overset` via `setImportOverset`. **`.puz`** (Publisher pack-and-go) is a CAB wrapping the `.pub`: a pure-TS MSCF reader (`src/lib/import/cab.ts`) unpacks it — STORED + MSZIP with cross-block dictionary continuation, honest `unsupported-compression` for Quantum/LZX — re-sniffs the extracted bytes (never the archived name) and re-enters the same pipeline. Verified live: a real 100 KB `.pub` in a 4-block stored CAB unpacks and converts identically to the raw file.
...
*Built (v1.9, P5):* the render diff is a real **fidelity harness** (`src/lib/import/fidelity.ts` + `corpus-fidelity.test.ts`). It parses the checked-in `pub2xhtml` reference renders (`fixtures/pub-refs/`, regenerated byte-stably by `scripts/refresh-corpus.mjs`), matches them to our `LayoutDocument` output deterministically (texts content-first then proximity; shapes and pattern-fills by bbox-center; no double-matching), and tallies the seven Markzware categories, gating each at ≥90% combined. **Measured 100% across all seven** on the content-bearing corpus with extras 0 / unmatched 0; 38 unit tests pin that each category can go red, so the gate is falsifiable rather than vacuous. `pub2xhtml` and our mapper both serialize the *same* libmspub parse, so this measures "our side drops and corrupts nothing" (the *nothing-silent* half of the exit gate), not an independent oracle for Publisher's own renderer — stated honestly in the harness header and the `fixtures/pub-refs/README.md`. The **adversarial samples** are `src/lib/import/{pub2raw,route,cab-adversarial}.test.ts` + sniff edges, exercising §10.1's POC-enforced controls (fake-binary timeout-kill and jail-wipe, size cap, extension-lie/OLE2 rejection, CAB bombs + hostile names, the prlimit rlimit wrapper's SIGXCPU classification), with a live-gated fuzz smoke behind `STP_LIVE_IMPORT=1`. **CI** (`.github/workflows/ci.yml`) runs the binary-free `checks`/`e2e` lanes on every push plus a `live-import` lane that installs libmspub-tools, regenerates the corpus, gates on drift, and runs the live suite.
...
| P1 *(✅ built, v1.5)* | Import service per §10.1's POC posture (Debian-slim image; fixture mode for binary-less dev/CI) and §10.7's swap seams: sniff → `pub2raw` → trace parser → geometry-only mapping | A `.pub` opens as correctly-sized, correctly-placed frames (the research doc's Milestone-1 bar) |
| P2 *(✅ built, v1.6)* | Text runs + styles + font remap; paths | Real content: text in the right boxes, right sizes; polygons faithful — schema v2 live with v1→v2 migration; §10.5 library vendored + lazy-loaded; corpus re-verified live (labels: 176/192 clean) |
| P3 *(✅ built, v1.7)* | Image extraction → assets + picture frames | Image-bearing publications convert — extracted images land in the same asset store L8's upload path ships. Corpus-corrected: bitmap fills (the real-world path) + graphic objects, content-hash dedupe, stretch fit; labels corpus 192/192 clean |
| P4 *(✅ built, v1.8)* | Import report UI + overset check + `.puz` handling | The associate sees exactly what to review — the design doc's §5.1 report requirement. Review panel with deep-linked notes/fonts/overset; font-load-gated overset measurement; pure-TS CAB (`.puz`) unpack (STORED+MSZIP), verified live |
| P5 *(✅ built, v1.9)* | Corpus + fidelity harness + adversarial security tests wired to CI | The ≥90% fidelity metric is measured, not asserted — **100% in all 7 Markzware categories** on the corpus, falsifiably (each category unit-tested going red); the §10.1 POC-enforced controls (incl. the now-built prlimit rlimit wrapper) pass an adversarial suite; CI runs binary-free `checks`/`e2e` lanes plus a `live-import` lane with a corpus drift gate |

*K-tranche exit gate:* L1–L15 e2e green on Konva; spike criteria met and recorded. *P-tranche exit gate* **✅ met (v1.9, P5):** corpus fidelity measured at 100% element-level across all seven Markzware categories (≥90% required); every degradation reported, nothing silent (tier notes + the falsifiable harness); the §10.1 **POC-enforced** controls pass the adversarial set (the production-deferred controls remain launch gates for the production tranche, not this one); the §10.7 swap seams hold as built (thin route, one-file subprocess seam, store-action entry, shared asset path, STUBS registered).
