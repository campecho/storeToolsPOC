...
2. **The server image stack is net-new, not shared.** The feature plan's proxy/recipe/re-render spine assumes a server image engine exists; the repo has none (no sharp/libvips, no ImageMagick, no libheif, and **no export/download path anywhere** — open/save/export is a ❌ across the POC). The `.pub` importer's `pub2raw.ts` subprocess seam is the only precedent and is the pattern PE copies wholesale: probe → jail → rlimit → timeout → honest degraded mode → diagnostics endpoint.
...
- **Trim and bleed guides** on the canvas whenever a target size is set — same visual language as the layout editor (dashed `#CC0000` bleed, `#9fb6df` guide blue, corner marks).
...
- **Suite consistency**: shared `AppHeader` rides along from the root layout; a `TitleBar` mirroring the layout editor's (back link, document name, experience-level switch, help); feedback context published to the store so `CapturedContextPanel` reads it.
