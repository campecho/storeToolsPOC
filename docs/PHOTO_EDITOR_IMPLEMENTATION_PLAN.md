...
*(One suite-consistency note for the doc owners, from the handoff review: the wires specify a real **Pro** level for this tool — curves, numeric entry — while the Layout Editor deliberately narrowed to two levels in its v1.3 because its Standard was already everything-visible. The two positions are compatible (Pro adds genuine controls here), but the contrast is worth an explicit line in the suite experience-model guidance so it reads as a decision, not drift. **Decided (v1.2): the Photo Editor ships all three levels; the Layout Editor's two-level model stands.** The suite guidance line is now needed, not optional — it should say that levels are defined per tool (Pro exists where a tool has genuine extra depth), that the level preference is per-tool state, not a suite-wide mode, and that a Pro associate crossing into a two-level tool — including mid-flight via the F2 round-trip — simply sees that tool's Standard, with no mapping surprise.)*
...
- **Client proxy surface: Canvas 2D `<canvas>`**, no library. Adjustments compile to per-channel LUTs + a color matrix applied in one pass over proxy `ImageData` (§3.3); Pro's sharpen/noise add a **bounded convolution stage** after the LUT pass (the one op family that isn't point math — PE0 benchmarks it); geometry uses canvas transforms. Proxy capped at ~2048 px long edge (≈2.6 MP) — small enough for <100 ms single-pass math on the ProDesk, large enough to judge a crop.
...
- **Upscale / background-removal / inpaint models: out of POC.** Async server-model jobs (Real-ESRGAN-class, rembg-class) are a hosting/cost/DPA decision (feature plan §5.5 #1) the POC can't settle. The POC ships the *seams* (async job contract, previewed-step UX the wires make a hard requirement) and one honest stand-in (§4 PE10).
...
- **Experience levels (E):** density only, never the file; **Simple** = Crop + Export rail, quick fixes + Auto-enhance, no contextual panel; **Standard** = the full Section-A surface (default); **Pro** = the same panels, denser — levels/curves-lite and numeric entry everywhere, no floating palettes, no workspace config (ships at PE9; the expanded controls are plan-specified per deviation #2). Progressive disclosure, never amputation.
...
2. **Pro ships — plan-specified where the wires stop** — product decision (v1.2): all three levels are in scope. The wires give Pro a Section-E miniature, the `PRO`-badged **More · levels, curves, sharpen, noise** row in Adjust, and the rule "same panels, denser — numeric entry everywhere"; the expanded More controls and numeric-entry layouts appear only as sketches in the low-fidelity miniature (a schematic curve, paired fields), never as spec'd in-panel controls. Those follow this plan's spec (PE9): **Levels** = black/white/gamma over a live luminance histogram; **Curves-lite** = one RGB curve, 3–5 draggable points; **Sharpen / Noise** = single amount sliders (unsharp mask / smoothing); numeric twin fields on every slider, plus numeric crop rect + straighten degrees. Flagged for a wireframe addendum (open question #8) — the build proceeds without blocking on it. Boundary: **capability-gated controls stay gated at every level** — Pro never unlocks CMYK-intent (dev #6), model Clean-up tools or upscale (dev #3), or the order/catalog affordances (devs #4–5); level is density, not entitlement. Until PE9 lands, the Pro segment renders disabled with an honest "coming" state. (The Layout Editor *dropped* Pro in its v1.3; the divergence is a recorded per-tool decision — §1.2 note.)
3. **Model-backed Clean up tools render disabled** — Remove object ships at stand-in quality (PE10); Spot heal, Red-eye, Remove background, and the "Fix an AI-generated file" card render in the tool grid **disabled, labeled "coming with the model service."** Same for the low-res rescue's Upscale button (the offer card renders; the action is disabled until hosting is decided).
...
src/components/photo-editor/
  ...
  panels/AdjustPanel.tsx                    → Auto-enhance · Light group · Color group (Warmth label) · "More …" PRO row (expands at Pro: Levels · Curves-lite · Sharpen · Noise, PE9; collapsed at Standard)
  panels/AdjustMoreSection.tsx              → the Pro More content: HistogramLevels · CurveEditor (3–5 points) · Sharpen/Noise sliders (dev #2 spec)
...
src/lib/photo/
  ...
  adjust-math.ts                            → LUT builders (brightness/contrast/exposure/highlights/shadows + levels/curve) + saturation/temperature matrices + sharpen/noise convolution kernels
...
`PhotoOp` = discriminated union (`op` tag): `crop{rect,ratio?,shape: rect|rounded|circle}` · `rotate{quarter|degrees}` · `flip{axis}` · `straighten{degrees}` · `resize{px|inchesAtDpi|percent}` · `adjust{param,value}` (param ∈ brightness/contrast/exposure/highlights/shadows/saturation/temperature — the UI label for temperature is **Warmth**) · `autoEnhance{computed params, stored explicit}` · the four **Pro ops, in v1 from day one** (UI lands PE9; no schema migration later): `levels{black,white,gamma}` · `curve{points[]}` · `sharpen{amount}` · `noiseReduce{amount}` · then `bleedExpand{strategy: mirror|smear|solid, amount}` · `fitToSize{mode,anchor}` · `textOverlay{...}` · `logoOverlay{assetId,...}` · `erase{maskAssetId}`. Every op carries a human label for the history dock (canonical strings per the wires, §5). Ops are **stored explicit** (auto-enhance writes the values it chose) so replay never re-derives.
...
Each step is one commit, demoable, with tests landing in the same commit. PE0–PE11 map the feature plan's E0–E8 and the handoff's Sections A–F onto POC reality (model-backed features are deferred seams, §6).
...
### PE9 — Pro level (Section E complete)

The Pro segment goes live and the third density tier ships. The **More** row in Adjust expands at Pro into `AdjustMoreSection`: **Levels** (black/white/gamma handles over a live luminance histogram), **Curves-lite** (one RGB curve, 3–5 draggable points — both compile into the same LUT pass as the Standard sliders), **Sharpen** and **Noise** (amount sliders; isomorphic convolution in `ops.ts`, benchmarked in PE0, applied after the LUT stage). **Numeric entry everywhere**: paired numeric fields on every slider, numeric crop rect + straighten degrees in Crop, numeric size fields in Fix for print (the Pro half of open question #4). Level switching stays density-only — same document, same recipe, nothing floats, no workspace config; Section E verified against the three miniatures (Simple = Crop+Export rail + quick fixes + Auto-enhance, no contextual panel; Standard = the Section-A surface; Pro = the same panels, denser). Capability-gated controls stay gated (dev #2 boundary). *Done when:* levels/curve/sharpen/noise round-trip client-preview → server-render within golden tolerance; every Pro control has a working numeric twin; switching levels never mutates the recipe or the history; a per-level e2e walks the same edit at all three densities; the convolution preview meets the PE0-measured budget or records its honest limit.

### PE10 — Clean up (honest stand-in)
...
### PE11 — Hardening & harness

`fixtures/photo-corpus/` finalized (phone JPEGs, HEICs incl. Live, low-res logo, screenshot, huge TIFF, AI-generated art, scanned doc; hostile set: polyglot, truncated, pixel-flood, zip-bomb-class PNG, SVG with entities/scripts — provenance noted per file, synthetic where real files aren't shippable). Perf budget gates in CI (open/adjust/export timings against recorded budgets, convolution included). Adversarial suite green against the jail. STUBS.md sweep; "Reset demo photo" affordance; README status update. *Done when:* CI runs unit + e2e + live lanes green including the photo suites; every budget is measured, met, or recorded as an honest limit.

---

## 5. Testing strategy

- **Unit (Vitest, colocated `*.test.ts`):** pure modules carry the weight — `adjust-math` per-parameter reference arrays (the Pro set included: levels/curve LUT builders, sharpen/noise kernels against reference outputs), `geometry` (crop/rotate/DPI — including the wires' worked examples: 672 @ 4×6 and 318 placed verify exactly; the 8×10/16×20 cases assert computed 120/56 with the wire's amber/red verdicts, open question #7), `fit` anchor matrix, `bleed` strategy pick across mirror/smear/solid, `sizes`, `pdf-wrap` (parse the emitted boxes back out of the bytes), schema + store (recipe cursor semantics, activeTool state machine, returnContext, merge/migration), `limits`, sniff/caps rejects.
...
- **E2E (Playwright):** `photo-editor.spec.ts` — open→crop→adjust→export happy path on the always-live core (no fixture mode needed); quick-fix navigation (Fix bleed/Fit to size → Fix for print; Convert format → Export); DPI chip click-through; history dock revert; the PE8 round-trip loop; the per-level pass (the same edit walked at Simple/Standard/Pro, PE9); oversize route-away; `data-testid` + `data-hydrated` conventions.
...
4. **Numeric resize entry** — the feature plan's P1 requires resize by pixels/inches-at-DPI/percent, but the wires fold sizing into Fit to size + target-size selection and show no numeric fields (Pro is "numeric entry everywhere"). *Assumption:* the `resize` op and its math ship (PE5) with a minimal numeric affordance inside Fix for print at Standard; the full numeric surface now ships with Pro at PE9 (v1.2). Standard's placement is still flagged for the designer — a wireframe answer would settle it.
...
8. **Pro control detail beyond the miniature** *(new in v1.2, with Pro now in scope)* — the wires specify Pro by rule ("same panels, denser — levels/curves-lite and numeric entry everywhere") plus the More row's four names; the expanded widgets are not drawn, and the Pro miniature's action bar shows two unlabeled extra buttons. *Proceeding with the deviation-#2 spec (histogram levels, one 3–5-point RGB curve, single amount sliders for sharpen/noise, numeric twins on every slider; the unlabeled extra action-bar buttons are omitted until the designer names them).* A small Pro wireframe addendum would be welcome before PE9, but nothing earlier blocks on it — Pro is sequenced last among the feature steps precisely so the addendum has time to land.

---

*Bottom line: the Photo Editor lands on the seams this POC has already proven — the jailed-native-engine pattern from `.pub` import, the schema/store/blob conventions from the layout editor, the honest-degradation pattern from fixture mode — and now builds to a real wireframe spec: the Section-A shell with its quick-fix bar and six-tile rail, panels and print-check states copied from the wires, and every designed-but-deferred control rendered visibly inert instead of silently missing. Three commits in (PE3) the architecture is proven end-to-end on real photos; three more (PE5) and the counter's daily grind — Fix bleed, Fit to size, honest DPI — is live; the round-trip with the Layout Editor (PE8) closes the suite loop; all three experience levels ship — Pro lands last (PE9) as density, not entitlement; everything model-shaped stays a labeled seam behind the preview-approve bar it will inherit.*
