...
src/lib/photo/
  ...
  pdf-wrap.ts                               → pure-TS single-image PDF writer: MediaBox/TrimBox/BleedBox + OutputIntent (GRACoL), DCT/Flate XObject, RGB & CMYK color spaces (cab.ts precedent)
...
**Export.** Export panel (rail task or Convert-format quick fix) → for text/logo overlays the client rasterizes each overlay at target resolution to PNG (fonts live client-side; keeps the server font-free — parity note in §5) → `RenderRequest{master, recipe, overlays, format, intent, printSafe}` → render host replays: sharp geometry → raw buffer → `ops.ts` pixel math → composites → color per the document intent (**CMYK through the committed GRACoL profile** via lcms `icc_transform`, or sRGB ICC embed) → encode (PDF via `pdf-wrap.ts` with trim/bleed boxes + OutputIntent) → bytes stream back → browser download. The canvas never freezes: the request is fire-and-forget with a progress chip in the status bar — the wire's own copy: "full-res render is queued server-side."
...
`fixtures/photo-corpus/` finalized (phone JPEGs, HEICs incl. Live, low-res logo, screenshot, huge TIFF, AI-generated art, scanned doc; hostile set: polyglot, truncated, pixel-flood, zip-bomb-class PNG, SVG with entities/scripts — provenance noted per file, synthetic where real files aren't shippable). Perf budget gates in CI (open/adjust/export timings against recorded budgets). Adversarial suite green against the jail. Experience levels finish to the Section-E spec (Simple = Crop+Export rail + quick fixes + Auto-enhance, no contextual panel; the two-segment switch verified against the miniatures, dev #2). STUBS.md sweep; "Reset demo photo" affordance; README status update. *Done when:* CI runs unit + e2e + live lanes green including the photo suites; every budget is measured, met, or recorded as an honest limit.
