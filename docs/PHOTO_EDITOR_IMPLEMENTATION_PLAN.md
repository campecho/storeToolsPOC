...
- **Unit (Vitest, colocated `*.test.ts`):** pure modules carry the weight — `adjust-math` per-parameter reference arrays, `geometry` (crop/rotate/DPI — including the wires' worked examples: 672 @ 4×6 and 318 placed verify exactly; the 8×10/16×20 cases assert computed 120/56 with the wire's amber/red verdicts, open question #7), `fit` anchor matrix, `bleed` strategy pick across mirror/smear/solid, `sizes`, `pdf-wrap` (parse the emitted boxes back out of the bytes), schema + store (recipe cursor semantics, activeTool state machine, returnContext, merge/migration), `limits`, sniff/caps rejects.
