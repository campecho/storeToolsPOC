...
| Fonts — Motiva Sans | `src/lib/layout/font-catalog.ts`, `public/fonts/README` | **Resolved 2026-08-26**: licensed WOFF2 files (300/400/500/700) ship in `public/fonts/` with `@font-face` in `globals.css`; system stack remains the fallback tail | The **import font library shipped in P2**: 8 libre stand-in families self-hosted under `public/fonts/` with lazy FontFace loading (`webfonts.ts`) and the remap table as data (`src/lib/import/font-remap.ts`). |
...
| Deployment access control | `src/middleware.ts`, `src/lib/access/`, `src/app/launcher/page.tsx` | **One shared static password** (user decision, 2026-09-08) over public Cloud Run services: middleware gates every route but `/launcher`, `/api/access` and `/fonts/*`; a correct password mints a 12h HMAC cookie keyed by the password itself. Unset `STP_ACCESS_PASSWORD` = no gate (dev, e2e, CI image lanes). The gate covers this app only — the prototype is a separate origin the cookie cannot reach, and it deploys ungated (user decision, 2026-09-10). | Real identity: IAM/IAP in front of the services, or the suite's SSO once it exists (`src/lib/identity.ts` is the other half of that swap). A shared password gives no per-user attribution and no revocation short of rotating it. See docs/DEPLOY_CLOUD_RUN_PLAN.md. |
...
| Curated font list | `src/lib/layout/text.ts` | In-store set TBD (Motiva Sans is licensed and shipped) |
